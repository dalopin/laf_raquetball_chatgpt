...
async function selectFurthestDate(page) {
  const options = await page.$$eval('#ddlDates option', opts =>
    opts.map((o, idx) => ({ idx, value: o.value, text: o.textContent }))
  );
...
async function selectEarliestTime(page) {
  ...
  if (!options.length) throw new Error('No time options available');
...
async function selectCourt(page) {
  const options = await page.$$eval('#cboCourtByTime option', opts =>
...
async function main() {
  ...
  try {
    ...
    const success = await saveReservation(page);
    if (!success) {
